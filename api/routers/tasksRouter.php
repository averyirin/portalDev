<?php

function tasksRouter($method, $page, $publicKey) {
    switch ($method) {
        case 'GET':
            if ($page[1]) {
                $session = new Session($publicKey, $signature, $params);

                //load single resource
                $task = Task::withID($page[1], $session);
                $data    = $task;
                $session->setHeader(200);
            } else {
                //retreive all resources with optional filters
                $status       = get_input('status', null);
                $task_type = get_input('task_type', null);
                $department_owner = get_input('department_owner', null);
                $createdAt    = get_input('created_at', null);
                $ownerGuid    = get_input('owner_guid', null);

                $status ? $params['status']       = $status : '';
                $task_type ? $params['task_type'] = $task_type : '';
                $department_owner ? $params['department_owner'] = html_entity_decode($department_owner) : '';
                $createdAt ? $params['created_at']   = $createdAt : '';
                $ownerGuid ? $params['owner_guid']   = $ownerGuid : '';

                $session = new Session($publicKey, $signature, $params);

                $tasks = Task::all($params, $session);

                $data = $tasks->getCollection();
                $session->setHeader(200);
            }

            header('Content-type: application/json');
            echo json_encode(array('status' => $status, 'data' => $data), 32);

            exit;
            break;
        case 'POST':
            if (get_input('action') == 'attachFile') {
                $session = new Session(null, null, null);
                if (Task::saveAttachments($_FILES['files'], $_POST['taskId'], $_POST['accessId'])) {
                    $session->setHeader(200);

                    $status = 'success';
                } else {
                    $session->setHeader(400);
                    $status = 'error';
                }
            } else {
                $payload            = json_decode(file_get_contents("php://input"), true);
                $payload['user_id'] = (int) $payload['user_id'];

                $session = new Session($publicKey, $signature, $payload);

                //check if edit or creation
                if ($page[1]) {
                    $task = Task::withID($page[1], $session);

                    if ($task) {
                        if ($task->can_edit) {
                            if ($task->validate()) {
                                if ($task->edit($payload)) {
                                    $data = $task;
                                    $session->setHeader(200);
                                } else {
                                    $session->setHeader(500);
                                    $status = 'error';
                                    $data   = array('message' => 'There was an error saving the task resource');
                                }
                            } else {
                                $session->setHeader(400);
                                $status = 'fail';
                                $data   = $task->errors;
                            }
                        } else {
                            $session->setHeader(401);
                            $status = 'fail';
                            $data   = array('message' => 'Insufficient access privledges');
                        }
                    } else {
                        $session->setHeader(404);
                        $status = 'error';
                    }
                } else {
                    $task = Task::withParams($payload);

                    if ($task->validate()) {
                        if ($task->create()) {
                            $session->setHeader(201);
                            $status = 'success';
                            $data   = array('id' => $task->id, 'accessId' => $task->access_id);
                            $task->sendEmail('submit');
                        } else {
                            $session->setHeader(500);
                        }
                    } else {
                        $session->setHeader(400);
                        $data = $task->errors;
                    }
                }
            }
            header('Content-type: application/json');
            echo json_encode(array('status' => $status, 'data' => $data), 32);

            exit;
            break;
        case 'PUT':
            $payload = json_decode(file_get_contents("php://input"), true);

            $session = new Session($publicKey, $signature, $payload);

            $task = Task::withID($page[1], $session);
            if ($task->update($payload)) {
                $session->setHeader(200);
                $status = 'success';
                $data   = array('id' => $task->id);
            } else {
                $session->setHeader(500);
                $data = $task->errors;
            }

            header('Content-type: application/json');
            echo json_encode(array('status' => $status, 'data' => $data), 32);

            exit;
            break;
        case 'DELETE':

            if ($page[1]) {
                $session = new Session($publicKey, $signature, $params);

                $task = Task::withID($page[1], $session);
                if ($task) {
                    $result = Task::delete($task);

                    if ($result) {
                        $session->setHeader(200);
                        $status = 'success';
                        $data   = null;
                    } else {
                        $session->setHeader(500);
                        $status = 'fail';
                    }
                } else {
                    $session->setHeader(404);
                    $status = 'error';
                }
            }

            header('Content-type: application/json');
            echo json_encode(array('status' => $status, 'data' => $data), 32);

            exit;
            break;
    }
}
